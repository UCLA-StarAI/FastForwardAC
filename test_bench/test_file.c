...
struct childList {
  ...
  double vr; // only used for naive propagation
...
// Naive backpropagation using O(n^2)
void naive_backpropagation(int index) {
  struct node *parent;
  struct childList *tempPtr;
  struct childList *p1;
  struct childList *p2;
  ...
  for (int i = index; i >= 0; i--) {
    ...
    /*Assign dr based on child position in cache*/
      ...
      p1 = parent->childHead;
      while (p1 != NULL) {
	int cIndex = p1->childIndex;
	p2 = parent->childHead;
	product = 1;
	while (p2 != NULL) {
	  if (p2->childIndex != cIndex){
	    product *= p2->vr; 
	  }
	  p2 = p2->next;
	}
	circuit[cIndex]->dr += product * parent->dr;
	p1 = p1->next;
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  for (int t = 0; t < iter; t++) {
	...
	if (*lineToRead == 'n') {
	  ...
	  while (sscanf(nodeList, " %d%n", &childIndex, &offset) == 1) {
	    children = (struct childList*)malloc(sizeof(struct childList));
	    children->childIndex = childIndex;
	    children->vr = circuit[childIndex]->vr;
  ...
  printf ("Your calculations took %.7lf seconds to run.\n", cpu_time_used );
